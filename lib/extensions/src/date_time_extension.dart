...
extension DateTimeEx on DateTime {
  ...
  String timeAgo(
    ...
    if (seconds < 5) {
      return LocaleKeys.second.tr;
    } else if (seconds < 60) {
      return '$seconds${abbreviatedUnits ? 's' : ' ${LocaleKeys.second.tr}'} ${LocaleKeys.ago.tr}';
    } else if (minutes < 60) {
      return '$minutes${abbreviatedUnits ? 'm' : ' ${LocaleKeys.minute.tr}'} ${LocaleKeys.ago.tr}';
    } else if (hours < 24) {
      return '${_formatUnit(hours, abbreviatedUnits, LocaleKeys.hour.tr)} ${LocaleKeys.ago.tr}';
    } else if (days < 30) {
      return '${_formatUnit(days, abbreviatedUnits, LocaleKeys.day.tr)} ${LocaleKeys.ago.tr}';
    } else if (days < 365) {
      final int months = _calculateMonths(now, this);
      return '${_formatUnit(months, abbreviatedUnits, LocaleKeys.month.tr)} ${LocaleKeys.ago.tr}';
    } else {
      final int years = days ~/ 365;
      return '${_formatUnit(years, abbreviatedUnits, LocaleKeys.year.tr)} ${LocaleKeys.ago.tr}';
    }
  }
