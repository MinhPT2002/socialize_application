...
import 'package:socialize_application/common/widget_components/loading/tween/delay_tween.dart';

enum CustomLoadingType { start, end, center }

class CustomLoading extends StatefulWidget {
  const CustomLoading({
    Key? key,
    this.color,
    this.type = CustomLoadingType.start,
    this.size = 45.0,
    this.itemBuilder,
    this.itemCount = 5,
...
class _CustomLoadingState extends State<CustomLoading>
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: SizedBox.fromSize(
        size: Size(widget.size * 1.15, widget.size),
        child: Row(
  ...
  List<double> getAnimationDelay(int itemCount) {
    switch (widget.type) {
      case CustomLoadingType.start:
        return _startAnimationDelay(itemCount);
      case CustomLoadingType.end:
        return _endAnimationDelay(itemCount);
      case CustomLoadingType.center:
      default:
